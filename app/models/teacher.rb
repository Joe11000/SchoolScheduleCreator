class Teacher < ActiveRecord::Base
  ...
  def format_teacher_schedule_for_week_calendar(teacher_id)
    teacher = Teacher.find_by_id(teacher_id)

    breaks_arr = Array.new(teacher.break_times.size)
    work_arr   = Array.new(teacher.courses_teaching.count)
    event_id_counter = 1;

    t.break_times.each_with_index do |i, break_time|
      breaks_arr[i] = {
                        "id"    => event_id_counter,
                        "start" => (break_time.start_time).to_i,
                        "end"   => (break_time.end_time).to_i,
                        "title" => ""
                      }
      event_id_counter += 1
    end

    t.courses_teaching.each_with_index do |i, course|
      work_arr[i] = {
                     "id" =>    event_id_counter,
                     "start" => (break_time.start_time).to_i,
                     "end" =>   (break_time.end_time).to_i,
    ...
    return { breaks: {events: breaks_arr}, work: {events: work_arr}}
  end
